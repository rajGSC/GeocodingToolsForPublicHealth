...
#
# Input file:   geocode-CO-provider-in.txt
# Output file:  geocode-CO-provider-out.txt
...
################################################################################
 ...
 basedir <- 'analysis/data'    ##### CHANGE BY PROJECT #####

 setwd(basedir)
# source("GoogleGeocode.R")

filename <- file.path("geocode-MO-offender-in-MASTER.txt")  ##### CHANGE BY PROJECT #####
...
if(file.exists("geocode-MO-offender-out.txt")){
write.table(d, file="geocode-MO-offender-out.txt",  ##### CHANGE BY PROJECT #####
            sep="\t", row.names=FALSE, append = T, col.names=F)
} else {
write.table(d, file="geocode-MO-offender-out.txt",  ##### CHANGE BY PROJECT #####
